...
def get_ip():
    iplist=request.urlopen('http://piping.mogumiao.com/proxy/api/get_ip_bs?appKey=0c0585edfeb84bec9ac65ff542c1e027&count=5&expiryDate=0&format=2').read().decode('utf8')
    return iplist
...
if __name__ == '__main__':
    ...
    while IPList.empty() == 0: #IPList.empty()返回false
        seconds +=1
        if IPList.qsize()==1: #如果队列还剩1，就调用获取IP的方法
            iplist = get_ip()
            for i in iplist.split(' '):
                IPList.put(i)
