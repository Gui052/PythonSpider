...
class WeiBoLogin(object):
    ...
    def get_html(self):
        html=self.session.get('http://weibo.com/6173176524/profile?topnav=1&wvr=6&is_all=1',headers=self.session.headers)
        #p=re.findall('<li class=\"WB_pic li_[\d*] S_bg1 S_line2 bigcursor\"[?:(.*?)][^>](.*?)</li>', html.text)
        #print(p)
        #url=li.find('img')['src']
        #with open('/home/aaron/桌面/图片','w') as f:
            #    f.write(f,requests.get(url).content)
        #print('导入成功')
        
if __name__ == "__main__":
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s\t%(levelname)s\t%(message)s")
    weibo = WeiBoLogin()
    weibo.login("18831299625", "2976351bbbb")
    weibo.get_html()
