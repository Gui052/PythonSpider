...
#------读取url-----
def read_url():
    file=open(os.path.join(sys.path[0],'tengxun.txt'),'r')
    urls=file.readlines()
    for i in urls:
        url.put(i[:-1])
#------写文件-------
def write_info(date,info,title):
    ...
    #gbk强行转码
    file=codecs.open(os.path.join(sys.path[0]+'/tengxun',dates+'.txt'),'w','utf-8')
    str=title+'\n'
    for i in info:
        str=str+i+'\n'
    file.write(str)
...
def get_info():
    while url.empty()==0:
        ...
        if len(date) == 0:  # 如果找不到日期直接结束
            continue
        title=code.xpath('//*[@id="Main-Article-QQ"]//h1/text()')
        info=code.xpath('//*[@id="Cnt-Main-Article-QQ"]//text()')
