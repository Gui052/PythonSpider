...
#------读取url-----
def read_url():
    file=open(os.path.join(sys.path[0],'wangyi.txt'),'r')
    urls=file.readlines()
    for i in urls:
        url.put(i[:-1])
#------写文件-------
def write_info(date,info,title):
    ...
    #gbk强行转码
    file=codecs.open(os.path.join(sys.path[0]+'/wangyi',dates+'.txt'),'w','utf-8')
    str=title+'\n'
    for i in info:
        str=str+i+'\n'
    file.write(str)
...
def get_info():
    while url.empty()==0:
        ...
        if len(date) == 0:  # 如果找不到日期直接结束
            continue
        title=code.xpath('//*[@id="epContentLeft"]/h1/text()')
        info=code.xpath('//*[@id="endText"]//text()')
        if len(title)==0:
            title=code.xpath('//*[@id="h1title"]/text()')
